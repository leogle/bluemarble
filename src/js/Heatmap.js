...
(function() {
    ...
    HeatmapOverlay.prototype.draw = function() {
        ...
        if (this.latlngs.length > 0) {
            ...
            while (len--) {
                var latlng = new BMap.Point(gridData[len].lng, gridData[len].lat);
     ...
     * add by lrh
     */
    HeatmapOverlay.prototype.processData = function() {
        var grid = this.buildGrid();
        var destData = [];
        var pData = this.toPixelData(this.data.data);
        for (var i = 0; i < grid.length; i++) {
            var result = this.interpolate(pData, grid[i].x, grid[i].y)
            var p = new BMap.Pixel(grid[i].x, grid[i].y);
            destData.push({
                lng: this._map.overlayPixelToPoint(p).lng,
                lat: this._map.overlayPixelToPoint(p).lat,
                count: result.count
            });
        }
        return destData;
    ...
    HeatmapOverlay.prototype.toPixelData = function(data) {
        var destData = [];
        for (var i = 0; i < data.length; i++) {
            var d = data[i];
            if (this._map.getBounds().containsPoint(new BMap.Point(d.lng, d.lat))) {
                var pixel = this._map.pointToOverlayPixel(new BMap.Point(d.lng, d.lat));
                destData.push({
                    x: pixel.x,
                    y: pixel.y,
                    count: d.count
    ...
    HeatmapOverlay.prototype.buildGrid = function() {
        var currentBounds = this._map.getBounds();
        var sw = currentBounds.getSouthWest();
        var ne = currentBounds.getNorthEast();
        var lb = this._map.pointToOverlayPixel(sw);
        var rt = this._map.pointToOverlayPixel(ne);
        var width = rt.x - lb.x;
        var height = lb.y - rt.y;
        var grid = [];
        var interval = this.conf.intervalPixel; //
        for (var i = 0; i <= width / interval; i++) {
            for (var j = 0; j <= height / interval; j++) {
                var x = lb.x + i * interval;
                var y = rt.y + j * interval;
    ...
    HeatmapOverlay.prototype.interpolate = function(srcData, x, y) {
            var count0 = 0,
                count1 = 0,
                data = {};
   ...
   */
    HeatmapOverlay.prototype.setDataSet = function(data) {
        ...
        this.setDataSetInner(data);
    }

    HeatmapOverlay.prototype.setDataSetInner = function(data) {
        var currentBounds = this._map.getBounds();
        var gridData = this.processData();
